...
jest.mock("../components/Reviews", () => ({ ratings }) => <div>Reviews: {ratings}</div>);
jest.mock("../components/EnrolledStudents", () => ({ enrolledStudents }) => <div>Enrolled Students: {enrolledStudents}</div>);
jest.mock("../components/Meetings", () => ({ meetings }) => <div>Meetings: {meetings}</div>);
...
describe("CourseTabs", () => {
    ...
    it("should render the meetings tab when clicked", () => {
        ...
        expect(screen.queryByText(/Reviews: 5 stars/i)).not.toBeInTheDocument();
        expect(screen.queryByText(/Enrolled Students:/i)).not.toBeInTheDocument();
        expect(screen.getByText(/Meetings: Meeting 1, Meeting 2/i)).toBeInTheDocument();
    });
