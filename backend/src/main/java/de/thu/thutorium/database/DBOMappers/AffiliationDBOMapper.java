...
/** A mapper class for converting {@link AffiliationTO} to {@link AffiliationDBO}. */
@Component
...
public class AffiliationDBOMapper {
  ...
  /**
   * Converts an {@link AffiliationTO} object to a {@link AffiliationDBO} object.
   *
  ...
  public AffiliationDBO toDBO(AffiliationTO affiliation) {
    ...
    Optional<UniversityDBO> universityDBO =
        universityRepository.findByUniversityName(affiliation.getUniversityName());
    if (universityDBO.isPresent()) {
      affiliationDBO.setUniversity(universityDBO.get());
    } else {
      throw new IllegalArgumentException(
          "University not found: " + affiliation.getUniversityName());
    }
