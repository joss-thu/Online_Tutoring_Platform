...
public class RatingTutorDBOMapper {
  ...
  public RatingTutorDBO toDBO(RatingTutorTO tutorRating) {
    ...
    UserDBO student =
        userRepository
            .findUserDBOByUserIdAndRoles_RoleName(tutorRating.getStudentId(), Role.STUDENT)
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "Student with id " + tutorRating.getStudentId() + " not found"));

    // Fetch the tutor and handle the case where the tutor is not found
    UserDBO tutor =
        userRepository
            .findUserDBOByUserIdAndRoles_RoleName(tutorRating.getTutorId(), Role.TUTOR)
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "Tutor with id " + tutorRating.getTutorId() + " not found"));
