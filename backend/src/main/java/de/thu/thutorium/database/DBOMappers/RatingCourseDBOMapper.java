...
public class RatingCourseDBOMapper {
  ...
  public RatingCourseDBO toDBO(RatingCourseTO courseRating) {
    ...
    UserDBO student =
        userRepository
            .findUserDBOByUserIdAndRoles_RoleName(courseRating.getStudentId(), Role.STUDENT)
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "Student with id " + courseRating.getStudentId() + " not found"));

    // Fetch the course and handle the case where the course is not found
    CourseDBO course =
        courseRepository
            .findCourseDBOByCourseId(courseRating.getCourseId())
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "Course with id " + courseRating.getCourseId() + " not found"));
