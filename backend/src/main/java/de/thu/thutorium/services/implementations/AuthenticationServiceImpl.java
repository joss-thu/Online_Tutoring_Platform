 ...
 * Provides methods to register and authenticate users and generate JWT tokens.
 */
@Component
...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public ResponseEntity<LogInResponseTO> authenticate(@RequestBody LogInRequestTO loginRequestTO) {
        ...
        authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(loginRequestTO.getEmail(), loginRequestTO.getPassword())
        );
        // Load user details
        ...
        // Create the response
        LogInResponseTO response = new LogInResponseTO(jwtToken, LocalDateTime.now(ZoneOffset.UTC), expiresIn);
        return ResponseEntity.ok(response);
    }

    /**
     * Registers a user based on the provided login request.
     * The code checks if the username (email) is registered under the specified role. If a username does not exist with
